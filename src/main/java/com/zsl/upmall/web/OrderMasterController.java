...
public class OrderMasterController {
    ...
    public JsonResult createOrder(@RequestBody CreateOrderVo orderInfo, HttpServletRequest request) {
        ...
        if (StringUtils.isBlank(orderInfo.getOrderSn())) {
            ...
            if (orderInfo.getCartId() - 0 == 0) {
                ...
                orderProductVoList.add(new OrderProductVo(sku.getSkuId(), sku.getShopId(), orderInfo.getProductCount(), sku.getSkuPrice(), sku.getSkuImage(), sku.getSpec(), sku.getSpuName(), sku.getSkuName()));
                ...
                for (ShoppingCart cart : shoppingCarts) {
                    ...
                    orderProductVoList.add(new OrderProductVo(sku.getSkuId(), sku.getShopId(), cart.getGoodsNum(), sku.getSkuPrice(), sku.getSkuImage(), sku.getSpec(), sku.getSpuName(), sku.getSkuName()));
            ...
            List<Integer> shopIdList = orderProductVoList.stream().map(OrderProductVo::getShopId).distinct().collect(Collectors.toList());
            for (Integer shopId : shopIdList) {
                //供应商拆单
                OrderShopMaster shopMaster = new OrderShopMaster();
                shopMaster.setOrderMasterId(orderId);
                shopMaster.setShopId(shopId);
                shopMaster.setOrderNo(generateOrderSn(userId));
                shopMaster.setCurrentState(SystemConfig.ORDER_STATUS_WAIT_PAY);
                shopMaster.setCreateTime(new Date());
                orderShopMasterService.save(shopMaster);
            }
            ...
            for (OrderProductVo orderProductVo : orderProductVoList) {
                ...
                orderDetail.setOrderId(orderShopMasterService.getOrderShopMasterId(orderId, orderProductVo.getShopId()));
    ...
    public JsonResult<IPage> updateOrderStatus(@PathVariable("id") Long id) {
        ...
        //判断订单状态是否为 待收货
        if (shopMaster.getCurrentState() - SystemConfig.ORDER_STATUS_RECIEVE != 0) {
            return result.error("订单状态不对");
        }

        // 设置订单已完成
        OrderShopMaster orderShopMaster = new OrderShopMaster();
        orderShopMaster.setId(shopMaster.getId());
        orderShopMaster.setCurrentState(SystemConfig.ORDER_STATUS_FINISH);
        if (!orderShopMasterService.updateById(orderShopMaster)) {
    ...
    public JsonResult<IPage> cancel(@PathVariable("id") Long id) {
        ...
        List<Long> list = orderShopMasterService.listOrderShopMasterId(id);
    ...
    public Object refundNotify(@RequestBody RefundNotifyVo refundNotifyVo) {
        ...
        List<Long> shopMasterId = orderShopMasterService.listOrderShopMasterId(order.getId());
        List<OrderDetail> orderDetails = new ArrayList<>();
        for (Long orderId : shopMasterId) {
            //判断是否拆单
            LambdaQueryWrapper<OrderDetail> orderDetailLambdaQueryWrapper = new LambdaQueryWrapper<>();
            orderDetailLambdaQueryWrapper.eq(OrderDetail::getOrderId, orderId);
            orderDetails.addAll(orderDetailService.list(orderDetailLambdaQueryWrapper));
        }
        ...
        if (!baseService.updateById(upOrderReceived)) {
            ...
            OrderShopMaster update = new OrderShopMaster();
            update.setCurrentState(SystemConfig.ORDER_STATUS_REFUNDED);
            orderShopMasterService.update(update, queryWrapper2);
        }
    ...
    public Object payNotify(@RequestBody PayNotifyVo payNotifyVo, String token) {
        ...
        //修改销量
        // 商品货品数量增加
        List<Long> list = orderShopMasterService.listOrderShopMasterId(order.getId());
        List<OrderDetail> orderDetails = new ArrayList<>();
        for (Long orderId : list) {
            QueryWrapper orderDetailWrapper = new QueryWrapper();
            orderDetailWrapper.eq("order_id", orderId);
    ...
    public JsonResult isBuyPackage(String orderSn, String sign) {
        ...
        if (orderMaster.getOrderStatus() - SystemConfig.ORDER_STATUS_WAIT_PAY == 0) {
            ...
            orderShopMasterQueryWrapper.eq("order_master_id", orderMaster.getId()).last("LIMIT 1");
            OrderShopMaster shopMaster = orderShopMasterService.getOne(orderShopMasterQueryWrapper);
            //判断订单详情sku数量是否为1
            QueryWrapper<OrderDetail> orderDetailQueryWrapper = new QueryWrapper();
            orderDetailQueryWrapper.eq("order_id", shopMaster.getId()).last("LIMIT 1");
